            ...
            <div className="h-56 w-11/12 rounded-3xl bg-[#131313] mt-5 mb-10 flex items-center overflow-x-scroll">
                {
                    searchResults.map((item) =>
                        <div>
                            <ProductCard name={item.product_name} image={item.image} brand={item.brand_name} desc={item.decription} state={item.address.state} city={item.address.city} price={ item.price} date=/>
                        </div>
